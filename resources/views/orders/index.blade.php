...
<body class="bg-gray-50 font-sans">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
                    ...
                    <!-- Kitchen Notification Bell -->
                    <div class="relative">
                        <button onclick="toggleNotificationPanel()" class="p-2 rounded-full bg-gray-100 hover:bg-gray-200 relative transition-colors">
                        ...
                        <!-- Notification Panel -->
                        <div id="notificationPanel" class="hidden absolute right-0 mt-2 w-72 bg-white rounded-md shadow-lg z-50 border border-gray-200">
                            <div class="p-3 border-b bg-gray-50">
                                <h3 class="font-medium text-gray-800">Kitchen Notifications</h3>
                            </div>
                            <div class="max-h-64 overflow-y-auto">
                                @forelse($kitchenNotifications as $notification)
                                <div class="p-3 border-b hover:bg-gray-50">
                                    <div class="flex justify-between">
                                        <span class="font-medium">Order #{{ $notification->order_id }}</span>
                            ...
                            <div class="p-2 border-t bg-gray-50 text-right">
                                <button onclick="markAllAsRead()" class="text-xs text-blue-600 hover:underline">Mark all as read</button>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-4 mt-2">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm text-gray-600">Priority: {{ $priorityCount ?? 0 }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <div class="relative">
                                <select id="rows-per-page" onchange="updateRowsPerPage(this.value)"
                                    class="bg-white border border-gray-300 rounded-md pl-3 pr-8 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <div class="flex items-center">
                                            Order ID
                                            <button onclick="sortTable('id')" class="ml-1 focus:outline-none">
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <div class="flex items-center">
                                            Date/Time
                                            <button onclick="sortTable('created_at')" class="ml-1 focus:outline-none">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                                                </svg>
                                            </button>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <div class="flex items-center">
                                            Total
                                            <button onclick="sortTable('total_price')" class="ml-1 focus:outline-none">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                                                </svg>
                                            </button>
    ...
    <script>
        ...
        document.addEventListener('click', function(event) {
            const notificationButton = document.querySelector('.relative button');
            const panel = document.getElementById('notificationPanel');

            if (!panel.contains(event.target) && event.target !== notificationButton && !notificationButton.contains(event.target)) {
        ...
        // Real-time updates for kitchen notifications (using Pusher or similar)
        @if(config('broadcasting.default') === 'pusher')
        window.Echo.channel('kitchen-channel')
            .listen('KitchenNotification', (data) => {
                // Update UI with new notification
                const notificationCount = document.querySelector('.relative button span');
        ...
        function showToast(message) {
            ...
            toast.className = 'fixed bottom-4 right-4 bg-blue-500 text-white px-4 py-2 rounded-lg shadow-lg transition-opacity duration-300 opacity-0';
